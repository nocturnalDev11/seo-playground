	...
	return (
		<>
			...
			<main className="max-w-3xl mx-auto px-4 py-10">
				<h1 className="text-3xl font-bold text-gray-900 mb-6 border-b pb-2">SEO Blog Posts</h1>
				...
				<ul className="space-y-4">
					{blogPosts.map((post) => (
						<li key={post.slug}>
							<Link
								href={`/blog/${post.slug}`}
								className="block p-4 rounded-xl border hover:bg-gray-50 transition shadow-sm"
							>
								<h2 className="text-xl font-semibold text-blue-600">{post.title}</h2>
								<p className="text-sm text-gray-500">Read post →</p>
							</Link>
						</li>
					))}
				</ul>
